...
const getMemberOrderPreData = async () => {
  if (query.addressId) {
    addressBuyNow.value = (await getMemberAddressByIdAPI(query.addressId)).result
  }
  if (query.skuId && query.count) {
    // 如果有skuId和count参数，说明是从商品详情页跳转过来的
...
//从立即购买过来并选择的地址
const addressBuyNow = ref<AddressItem>()
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !selectedAddress?.id }" @tap="onOrderSumbit">
      提交订单
    </view>
  </view>
</template>
